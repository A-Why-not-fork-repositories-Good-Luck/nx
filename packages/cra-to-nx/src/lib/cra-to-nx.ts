...
async function reorgnizeWorkspaceStructure(options: NormalizedOptions) {
  ...
  if (options.isVite) {
    const indexPath = options.nested
      ? 'index.html'
      : `apps/${options.reactAppName}/index.html`;
    const oldIndexPath = options.nested
      ? 'public/index.html'
...
function moveFilesToTempWorkspace(options: NormalizedOptions) {
  ...
  const requiredCraFiles = [
    'project.json',
    options.nested ? null : 'package.json',
    'src',
    'public',
...
async function addBundler(options: NormalizedOptions) {
  if (options.isVite) {
    ...
    addViteCommandsToPackageScripts(options.reactAppName, options.nested);
    writeViteConfig(options.reactAppName, options.nested, options.appIsJs);
    writeViteIndexHtml(options.reactAppName, options.nested, options.appIsJs);
    renameJsToJsx(options.reactAppName, options.nested);
  } else {
    ...
    addCracoCommandsToPackageScripts(options.reactAppName, options.nested);

    writeCracoConfig(options.reactAppName, options.isCRA5, options.nested);
...
function cleanUpUnusedFilesAndAddConfigFiles(options: NormalizedOptions) {
  ...
  cleanUpFiles(options.reactAppName, options.nested);

  output.log({ title: "📃 Extend the app's tsconfig.json from the base" });

  setupTsConfig(options.reactAppName, options.nested);

  if (options.e2e && !options.nested) {
